    ...
    Drupal.behaviors.pdfBookmarks = {
        attach: function (context, settings) {
            ...
            function initBookmarks()
            {
                ...
                if (bookmarksToggle) {
                    bookmarksToggle.addEventListener(
                        'click', function (e) {
                            e.preventDefault();
                            toggleBookmarks();
                        }
            ...
            function loadBookmarks()
            {
                if (typeof window.pdfViewerInstance !== 'undefined' && window.pdfViewerInstance.pdfDoc) {
                    window.pdfViewerInstance.pdfDoc.getOutline().then(
                        function (outline) {
                ...
                try {
                    ...
                    if (typeof window.pdfViewerInstance !== 'undefined' && window.pdfViewerInstance.pdfDoc) {
                        if (typeof dest === 'string') {
                            window.pdfViewerInstance.pdfDoc.getDestination(dest).then(
                                function (destArray) {
            ...
            function goToDestination(destArray)
            {
                ...
                if (typeof window.pdfViewerInstance !== 'undefined' && window.pdfViewerInstance.pdfDoc) {
                    window.pdfViewerInstance.pdfDoc.getPageIndex(destArray[0]).then(
                        function (pageIndex) {
