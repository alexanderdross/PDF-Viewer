        ...
        attach: function (context, settings) {
            ...
            function saveReadingProgress()
            {
                ...
                if (drupalSettings.user && drupalSettings.user.uid > 0) {
                    ...
                    fetch(
                        Drupal.url('api/pdf-embed-seo/v1/documents/' + documentId + '/progress'), {
                            method: 'POST',
                            headers: {
            ...
            function showProgressPrompt(pageNum)
            {
                ...
                prompt.querySelector('.pdf-progress-resume').addEventListener(
                    'click', function () {
                        resumeReading();
                    }
                );

                prompt.querySelector('.pdf-progress-start-over').addEventListener(
                    'click', function () {
                        startOver();
                    }
                );
